...
# Generated by Gemini
def chebyshev_interpolate_jax(x, a, b, weights, rounding_eps=1e-5, clip_end=False):
    """
    Chebyshev interpolation in JAX.
    """
    out_dim, in_dim, n_weights = weights.shape
    num_grid_intervals = n_weights - 1
    batch_size = x.shape[0]
    ...
    basis = compute_chebyshev_basis(x, a, b, degree=num_grid_intervals)
...
def cheby_summation_oversampled_refit(old_weights, old_a, old_b, new_a, new_b, degree, oversample_ratio=1.5):
    ...
    basis = compute_chebyshev_basis(x_phys, old_a, old_b)
...
def refit_weights_and_counts_jax(weights,
    ...
    if basis_type == "bspline":
        ...
        new_weights = cheby_summation_oversampled_refit(weights, a, b, new_a, new_b, new_num_grid_intervals, oversample_ratio=1.5)
    else:
...
def _stretch_weights_and_counts_jax(combined_mask, activation_weights, data_counts, ood_counts, a, b, ood_a, ood_b, k, rounding_eps, exact_stretch, basis_type):
    ...
    num_grid_intervals = activation_weights.shape[-1] - k
